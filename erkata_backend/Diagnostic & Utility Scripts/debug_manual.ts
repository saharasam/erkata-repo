...
// Use the IP address that worked before
const IP_URL =
  'postgresql://postgres.mpufgkovosjvrufxkckw:raqznkrVLNouGEze@18.202.64.2:6543/postgres?pgbouncer=true&sslmode=require&connection_limit=5';
  ...
  try {
    ...
    if (match) {
      ...
      allMatches.forEach((m) => {
        console.log(
          `  - Match ${m.id} [${m.status}] -> Req: ${m.request.description}`,
        );
      });
    } else {
